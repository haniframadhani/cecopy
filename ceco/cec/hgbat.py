...
class Hgbat(Benchmark):
    """
    A class representing the Hgbat benchmark function, used for optimization problems.

    The Hgbat function is a highly multimodal, non-separable, and scalable function.
    It is defined as:
    f(X)=\| (sum_{i=1}^{D}x_i^2)^2-(sum_{i=1}^{D}x_i)^2 \|^{1/2}+(0.5 sum_{i=1}^{D}x_i^2 + sum_{i=1}^{D}x_i)/ D+0.5
    ...
    def __init__(self, rotation: list[list[float]], shift: list[float], f_bias: float = 0) -> None:
        """
        Initializes the Hgbat class with a rotation matrix, a shift vector, and a bias term.
    ...
    def evaluate(self, input_vector: list[float]) -> float:
        """
        Evaluates the Hgbat function for the given input vector after applying shift and rotation transformations.

        Parameters:
            input_vector (list of float): The input vector [x1, x2, ..., xD].

        Returns:
            float: The result of the Hgbat function after applying rotation and shift.
        ...
        term1 = abs(sum_x_squared ** 2 - sum_x ** 2) ** (1/2)
        term2 = (0.5 * sum_x_squared + sum_x) / dimension
